...
export default function HomeView({ transactions, allTransactions, hiddenCount = 0, onAddTransactions, onDeleteTransaction, onBulkDelete, onEditTransaction, onSeeAll, userName = "there", activeSpace, logDisabled, onUpgrade }: HomeViewProps) {
  ...
  async function handleSend() {
    ...
    const ambiguousMatch = text.match(/^(\d+(?:\.\d+)?(?:k|K|l|L)?)\s+([a-zA-Z]+(?:\s+[a-zA-Z]+)?)$/) || text.match(/^([a-zA-Z]+(?:\s+[a-zA-Z]+)?)\s+(\d+(?:\.\d+)?(?:k|K|l|L)?)$/);
    if (ambiguousMatch) {
          ...
          <>
            ...
            {hiddenCount > 0 ? (
              <div className="px-3 py-3">
                <button
                  onClick={onUpgrade}
                  className="w-full flex items-center gap-3 px-4 py-4 rounded-2xl active:opacity-70"
                  style={{ background: "rgba(200,49,255,0.06)", border: "1px dashed rgba(200,49,255,0.25)" }}
                >
                  <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: "rgba(200,49,255,0.12)" }}>
      ...
      <>
        ...
        <div className="fixed bottom-0 left-0 right-0 z-[500] max-w-[430px] mx-auto rounded-t-[28px] flex flex-col" style={{ background: "var(--md-surface)", paddingBottom: "calc(env(safe-area-inset-bottom,0px) + 24px)" }}>
          ...
          <div className="px-6 pt-3 pb-4">
            ...
            <div className="text-sm" style={{ color: "var(--md-on-surface-variant)" }}>{activeSpace.name} · {activeSpace.people_count} people</div>
          </div>

          {/* Total row */}
          <div className="mx-4 mb-4 px-4 py-3 rounded-2xl flex items-center justify-between" style={{ background: "var(--md-surface-container-low)" }}>
          ...
          <div className="px-4 flex flex-col gap-2 mb-4">
            {Array.from({ length: activeSpace.people_count }).map((_, i) => {
              const share = Math.round(totalExpense / activeSpace.people_count);
              const isLast = i === activeSpace.people_count - 1;
              // Last person gets remainder to avoid rounding gaps
              const amount = isLast ? totalExpense - share * (activeSpace.people_count - 1) : share;
              return (
                <div key={i} className="flex items-center gap-3 px-4 py-3 rounded-2xl" style={{ background: "var(--md-surface-container-low)" }}>
                  ...
                  <span className="text-sm font-semibold" style={{ color: "var(--md-on-surface)" }}>{fmtFull(amount)}</span>
                </div>
              );
            })}
          </div>
          ...
          {/* Footer note */}
          <div className="mx-4 px-4 py-3 rounded-2xl text-center text-xs" style={{ color: "var(--md-on-surface-variant)", background: "var(--md-surface-container-lowest)" }}>
            Equal split · {fmtFull(Math.round(totalExpense / activeSpace.people_count))} per person
          </div>
        </div>
      </>
    )}
