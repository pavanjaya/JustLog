...
export default function Drawer({ open, view, onClose, onNavigate, onDeleteAll, user, subStatus, validUntil, onUpgrade, onExport, isPro }: DrawerProps) {
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 px-3 py-3 flex flex-col gap-1">
          ...
          <DrawerItem icon={<IconExport />} label="Export Data" sublabel={isPro ? undefined : "Pro only"} onClick={() => { onClose(); if (!isPro) { onUpgrade?.(); return; } onExport?.(); }} />
...
function DrawerItem({ icon, label, sublabel, onClick, danger, active }: { icon: React.ReactNode; label: string; sublabel?: string; onClick: () => void; danger?: boolean; active?: boolean }) {
  return (
    <button
      onClick={onClick}
      className="w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl text-left md-ripple transition-colors"
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="text-[15px] font-medium" style={{ color: danger ? "var(--md-error)" : active ? "var(--md-primary)" : "var(--md-on-surface)" }}>{label}</div>
        {sublabel && <div className="text-[12px] mt-0.5" style={{ color: "var(--md-on-surface-variant)" }}>{sublabel}</div>}
      </div>
    </button>
  );
}
